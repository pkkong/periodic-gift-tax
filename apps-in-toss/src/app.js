...
const PDF_FILE_NAME = "우리-아기-증여-준비-자료.pdf";
...
const scheduleDisclosure = scheduleSummary?.closest(".result-disclosure");
...
function renderSafeBriefing(input, tax) {
  ...
  safeBreakdown.innerHTML = `
    <div>
      <span>기본공제</span>
      <strong>${formatWon(tax.availableDeduction)}</strong>
      <small>${getDeductionLawCopy(input)}</small>
    </div>
    <div>
      <span>과세표준 50만원 미만</span>
      <strong>부과 제외</strong>
      <small>상속세 및 증여세법 제55조 제2항은 과세표준이 50만원 미만이면 증여세를 부과하지 않는다고 정하고 있어요.</small>
    </div>
  `;
...
function getDeductionLawCopy(input) {
  if (input.relationshipType === "parent_minor_child" || input.relationshipType === "grandparent_minor_grandchild") {
    return "상속세 및 증여세법 제53조 제2호는 직계존속에게 증여받는 경우 5천만원을 공제하되, 수증자가 미성년자이면 2천만원을 공제한다고 정하고 있어요.";
  }
  if (input.relationshipType === "parent_adult_child" || input.relationshipType === "grandparent_adult_grandchild") {
    return "상속세 및 증여세법 제53조 제2호는 직계존속에게 증여받는 경우 5천만원을 공제한다고 정하고 있어요.";
  }
  if (input.relationshipType === "adult_child_parent") {
    return "상속세 및 증여세법 제53조 제3호는 직계비속에게 증여받는 경우 5천만원을 공제한다고 정하고 있어요.";
  }
  return "상속세 및 증여세법 제53조는 증여자와 수증자의 관계별 증여재산공제 한도를 정하고 있어요.";
}

function getDeductionLawSummary(input) {
  if (input.relationshipType === "parent_minor_child" || input.relationshipType === "grandparent_minor_grandchild") {
    return "상증세법 제53조 제2호, 미성년자 2천만원";
  }
  if (input.relationshipType === "adult_child_parent") {
    return "상증세법 제53조 제3호, 5천만원";
  }
  return "상증세법 제53조 제2호, 5천만원";
}
...
function renderModeHints(input) {
  ...
  modeHint.textContent = input.giftMode === "lump_sum"
    ? `한 번에 보내는 경우도 신고 전 홈택스에서 공제 기준을 다시 확인하세요.`
    : `오늘 기준 대표 예시는 10년 동안 매월 약 ${formatWon(recommendedMonthly)}이에요.`;
  amountTitle.textContent = input.giftMode === "lump_sum" ? "한 번에 보내기 기준을 확인하세요" : "매월 보내기 기준을 확인하세요";
...
function renderResults(input, valuation, tax, errors, warnings) {
  ...
  const needsReview = tax.taxBase >= 500_000;

  resultStatus.classList.toggle("has-tax", needsReview);
  resultStatus.classList.toggle("has-error", hasErrors);
  resultStatusLabel.textContent = hasErrors ? "입력 확인 필요" : needsReview ? "별도 확인 필요" : "신고 준비 기준 확인";
  resultTaxAmount.textContent = hasErrors
    ? "다시 확인해주세요"
    : needsReview
      ? "홈택스에서 최종 확인하세요"
      : "공제 기준 안에서 준비해요";
  resultLead.textContent = hasErrors
    ...
    : needsReview
      ? "입력한 조건이 공제 기준을 넘을 수 있어요. 신고 전 홈택스나 전문가 확인이 필요해요."
      : "기본공제와 과세최저한 기준을 확인했어요. 실제 신고 전 홈택스에서 최종 확인하세요.";
  ...
  const metrics = [
    {
      label: "안내 기준",
      value: formatLimit(safeLimit),
      sub: "기본공제와 과세최저한 기준"
    },
    {
      label: "증여 방식",
      value: input.giftMode === "periodic" ? "매월 보내기" : "한 번에 보내기",
      sub: input.giftMode === "periodic" ? "대표 월 금액은 안내 기준으로만 제공" : "현금 일시증여 준비"
    },
    {
      label: "신고기한",
  ...
  resultFilingMeta.innerHTML = [
    ...
    ["기본공제 근거", getDeductionLawSummary(input)],
    ["과세최저한 근거", "상증세법 제55조 제2항"],
  ...
  resultDocsSummary.textContent = input.giftMode === "periodic"
    ? "준비 체크리스트와 증여 약정서 초안을 저장해요."
    : "준비 체크리스트와 현금 증여 확인서 초안을 저장해요.";

  scheduleRows.innerHTML = "";
  scheduleSummary.textContent = "기준 내역 보기";
  if (scheduleDisclosure) scheduleDisclosure.hidden = true;
...
function renderExecutionGuide(input, tax) {
  ...
  if (input.giftMode === "periodic") {
    ...
    executeSummary.textContent = isSimulation
      ? "지금은 매월 보내기 신고 준비 흐름만 이어갈게요."
      : "정한 기준에 맞춰 첫 이체를 보내고, 앞으로 같은 방식으로 기록을 남기세요.";
    executeTransferNote.textContent = isSimulation
      ? "실제 신고 전에는 수증자 명의 계좌와 첫 이체내역을 준비하세요."
      : "정기증여 약정과 첫 이체내역을 함께 보관하세요.";
  } else {
    executeTransferTitle.textContent = isSimulation
      ? `${recipientName} 명의 계좌가 필요해요`
      : `${recipientName} 계좌로 이체하세요`;
    executeSummary.textContent = isSimulation
      ? "지금은 한 번에 보내기 신고 준비 흐름만 이어갈게요."
...
function renderHometaxGuide(input, valuation, tax) {
  ...
  const preparedItems = [
    "신고 준비 체크리스트",
    isPeriodic ? "정기증여 약정서 초안" : "현금 증여 확인서 초안",
    "기본공제와 과세최저한 근거 메모"
  ];
  const neededItems = [
    "가족관계증명서 또는 기본증명서",
  ...
  if (input.priorSameDonorGiftValue > 0) {
    preparedItems.push("최근 10년 동일인 증여 확인 메모");
    neededItems.push("이전 증여 신고서 또는 홈택스 결정정보 대조");
  }
  if (tax.generationSkippingRate > 0) {
    preparedItems.push("세대생략 증여 가능성 확인 메모");
    neededItems.push("손자녀 증여 관계를 확인할 가족관계 자료");
  }

  preparedSummary.textContent = isPeriodic ? "정기증여 준비자료를 만들어요." : "일시증여 준비자료를 만들어요.";
  ...
  const assetDetail = isPeriodic
    ? "매월 보내기는 홈택스에서 현금 증여와 정기 지급 약정 내용을 실제 이체내역에 맞춰 확인하세요."
    : "한 번에 보내기는 홈택스에서 현금 증여 금액과 실제 이체일을 맞춰 확인하세요.";
  hometaxAssetGuide.textContent = `${assetDetail} 기본공제와 과세표준 50만원 미만 부과 제외 여부는 홈택스 산출 화면에서 최종 대조하세요.`;
}
...
function renderValidation(input, errors, warnings, tax) {
  ...
  if (tax.generationSkippingTax > 0) {
    notices.push({
      type: "warning",
      text: "수증자가 증여자의 자녀가 아닌 직계비속이면 세대생략 할증이 적용될 수 있어요."
    });
  }
...
function renderBabyMascot(stepKey, progressIndex, progressTotal) {
  ...
  const states = {
    ...
    amount: ["happy", "거의 끝", "신고 준비 기준을 확인해요."],
    result: ["proud", "준비 완료", "이제 할 일만 챙기면 돼요."],
...
function getNextButtonText(stepKey) {
  ...
  if (stepKey === "giftMode") return "준비 결과 보기";
...
function renderAmountGuard(valuation) {
  ...
  amountGuard.innerHTML = `
    ...
    <p>현재 조건은 이 관계의 안내 기준 ${formatLimit(safeLimit)}을 넘을 수 있어요.</p>
    ${amountTaxOverrideApproved ? "<p>결과 화면에서 신고 준비물을 볼게요.</p>" : '<button class="guard-action" type="button" data-continue-tax>이대로 결과 보기</button>'}
  `;
  updateWizardNavState();
}
...
async function createPdfBase64() {
  if (!documentPreview.innerHTML.trim()) {
  ...
  renderRoot.innerHTML = documentPreview.innerHTML;
  document.body.append(renderRoot);

  try {
    const pages = Array.from(renderRoot.querySelectorAll(".print-page"));
    if (!pages.length) throw new Error("Document page is missing");
