...
| --- | --- | --- |
| Local webapp-clone candidate | `019ecbaa-5aad-7b43-89e0-9bb270391877` | Built locally on `2026-06-15 KST` as `baby-gift-tax-helper.ait`, but not yet uploaded to Console. This is the current source direction: public-webapp UX clone plus policy changes only. |
...
- Later on `2026-06-15 KST`, the user clarified the intended approach again: first clone the public webapp UI/UX/design into Apps in Toss and only then patch the rejection reasons. The local source was rebuilt from the public webapp files under `apps-in-toss/`, with only Apps in Toss path/PDF changes and the policy patches listed above. A local `.ait` was built successfully with deploymentId `019ecbaa-5aad-7b43-89e0-9bb270391877`, but it has not been uploaded to Console yet in this source state.
