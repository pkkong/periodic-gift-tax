...
- `apps-in-toss/`: separate Apps in Toss WebView miniapp. As of `2026-06-15 KST`, the current local candidate is a public-webapp UX clone with policy changes only: it keeps the landing, progressive wizard, mascot, result, execution, HomeTax, and PDF flow, but removes the visible amount-entry calculation page and estimated-tax result copy.
...
- `apps-in-toss/index.html`: Apps in Toss shell copied from the public webapp and patched for Vite/WebView paths. It does not register the public PWA manifest or service worker.
- `apps-in-toss/src/app.js`: Apps in Toss wizard logic copied from the public webapp, with the `amount` step removed from `STEPS`, result copy changed from tax estimates to filing-prep guidance, and Apps in Toss PDF generation using `html2canvas`, `jspdf`, `saveBase64Data`, and `openPDFViewer`.
- `apps-in-toss/src/styles.css`: Apps in Toss styling copied from the public webapp, with small adjustments for legal-copy cards and offscreen PDF rendering.
- `apps-in-toss/src/tax.js`: Apps in Toss copy of the pure tax engine. The current miniapp uses it internally for safe-limit guidance, filing deadline, and representative monthly defaults only; it should not expose a user-entered tax calculator in the Apps in Toss UI unless policy direction changes.
- `apps-in-toss/src/documents.js`: Apps in Toss PDF pack. It contains a filing-prep checklist, law-basis memo, HomeTax prep checklist, and blank optional 증여 약정서/확인서 초안. It must not include payable-tax tables, calculation tables, or phrases that present a confirmed tax amount.
...
- Apps in Toss policy candidate exception: the public webapp flow above still applies to the GitHub Pages app, but the current `apps-in-toss/` candidate removes the visible amount-entry step after `한 번에 보내기` / `매월 보내기`. The mode step should go straight to result/prep guidance, and result copy must not say `예상 세금 0원`, `예상 납부세액이 0원이에요`, or any equivalent zero-tax guarantee.
- Apps in Toss safe-limit step must include the law basis under the 2,050만원 안내: `기본공제` with 상속세 및 증여세법 제53조 relationship copy, and `과세표준 50만원 미만` with 상속세 및 증여세법 제55조 제2항 copy.
- Apps in Toss account choice copy currently uses `아직 없지만 시뮬레이션 할게요` with a space before `할게요`, per the user's requested wording.
...
- Apps in Toss source was re-cloned from the public webapp on `2026-06-15 KST` after the user objected that the React policy candidate felt like a different app. The old React `apps-in-toss/src/main.tsx` and `apps-in-toss/src/index.css` were removed. Do not revive that simplified React UI unless the user explicitly asks for a different app experience.
- Apps in Toss WebView may not respond to `window.print()`. Keep the miniapp PDF path in `apps-in-toss/src/app.js`: it renders `#documentPreview` pages with `html2canvas`, creates a PDF with `jspdf`, then calls `saveBase64Data` for `PDF 저장` and `openPDFViewer` for preview, with browser download fallback for local QA.
...
| --- | --- | --- |
| `20260616-9` | `019ecbaa-5aad-7b43-89e0-9bb270391877` | Current review target. Uploaded on `2026-06-16 KST` from commit `c60518f` (`apps-in-toss/webapp-clone-policy-20260615`) as the public-webapp UX clone with policy changes only. Console push test sent. On `2026-06-18 KST`, the user reported app info approval, but release review was not submitted because the reusable Console tab was locked by an earlier Chrome automation session and a new Console tab redirected to Toss Business sign-in. |
| `20260615-8` | `019ecb91-f473-7225-9436-254474e225ae` | Uploaded simplified React policy candidate from commit `1b69417`; superseded by the local webapp-clone candidate unless the user explicitly returns to it. Console push test sent; release review blocked by pending app-info review. |
| `20260615-7` | `019ecb88-ac50-7f9a-8359-a5afc0571d86` | Uploaded simplified React policy candidate from commit `e3d5183`; superseded by the local webapp-clone candidate unless the user explicitly returns to it. Console push test sent; release review blocked by pending app-info review. |
...
- Local webapp-clone Apps in Toss QA passed on `2026-06-15 KST` in the Codex in-app browser at `http://127.0.0.1:5173/`: landing and result screens had no horizontal overflow at 360px, 390px, and 430px; the safe-limit step showed `기본공제`, 상속세 및 증여세법 제53조 제2호, `과세표준 50만원 미만`, and 상속세 및 증여세법 제55조 제2항; `giftMode` went directly to `result`; `매월 얼마를 보낼까요?`, `예상 세금 0원`, and `예상 납부세액이 0원이에요` were absent from visible text; PDF save generated four `.print-page` pages and showed `PDF 저장을 열었어요.` with no browser console errors.
...
- The user then asked whether the PDF should include a gift contract. Commit `1b69417` adds a second PDF page, `현금 증여 계약서/확인서 초안`, as a blank optional reference form and explicitly avoids presenting it as required or legally guaranteed. It was uploaded as build `20260615-8`, deployment `intoss-private://baby-gift-tax-helper?_deploymentId=019ecb91-f473-7225-9436-254474e225ae`, with memo `정책 반영 후보: 세액 계산 입력과 예상 세액 결과를 제거하고, 공제 기준 안내, 매월 보내기 대표 예시, 홈택스 준비 순서, PDF 체크리스트와 현금 증여 계약서 초안을 제공합니다.`
- Build `20260615-8` reached `검토 필요`, its Console push test was sent, and its `검토 요청` button became enabled. Clicking `검토 요청` still showed `앱 정보 검토를 먼저 완료해 주세요`, so release review remains blocked by the pending app-info review.
- Later on `2026-06-15 KST`, the user clarified the intended approach again: first clone the public webapp UI/UX/design into Apps in Toss and only then patch the rejection reasons. The local source was rebuilt from the public webapp files under `apps-in-toss/`, with only Apps in Toss path/PDF changes and the policy patches listed above. Commit `c60518f` / tag `apps-in-toss/webapp-clone-policy-20260615` is this source state. It was uploaded to the Apps in Toss Console on `2026-06-16 KST` as build `20260616-9`, deployment `intoss-private://baby-gift-tax-helper?_deploymentId=019ecbaa-5aad-7b43-89e0-9bb270391877`. Console push test was sent.
- On `2026-06-16 KST`, app info was re-submitted after changing the subtitle to `아이 증여 신고 준비` and updating the detailed description to explicitly state that amount-entry tax calculation, confirmed tax guidance, tax agency, HomeTax auto submission, and server storage are not provided. The app info page showed `검토 중이에요. 결과는 영업일 기준 2일 내 이메일로 알려드릴게요.` and `검토를 요청했어요.`. A release review attempt for build `20260616-9` still showed `앱 정보 검토를 먼저 완료해 주세요` / `앱 정보가 승인되어야 앱을 출시할 수 있어요.`, so release review was blocked until app info approval.
- On `2026-06-18 KST`, the user reported that app info was approved and asked to proceed with release review. The agent attempted to use the existing Chrome Console tab, but that tab remained locked to an earlier Chrome automation session (`Tab 394117758 is already part of browser session 019ed5a0-8f87-7513-8628-3c0312c641da`). Opening a fresh Console tab redirected to Toss Business sign-in, and the attempted `kongncompany@naver.com` login showed `INVALID_ACCOUNT`. The Apps in Toss `ait` CLI only supports token, build, deploy, and migrate; it has no release-review submission command. Therefore release review for `20260616-9` still needs a fresh logged-in Console session, then click `검토 요청` on the `20260616-9` row.
