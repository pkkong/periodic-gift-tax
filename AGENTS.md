...
- On `2026-06-18 KST`, after the user logged in again, release review for `20260616-9` was submitted. The review modal release note said the app keeps the public webapp UX while removing amount-entry tax calculation and confirmed tax guidance, and provides law-basis guidance, filing deadline guidance, post-transfer checklist, HomeTax input order, PDF checklist, and blank gift agreement/confirmation drafts. Registered in-app feature: Korean `증여 신고 준비하기`, English `Gift Prep`, URL `/`. The Console changed the build row to `검토 중`, changed the action to `요청 취소`, and showed `요청이 완료되었어요. 검토 후 이메일로 알려드릴게요. (20260616-9)`.
