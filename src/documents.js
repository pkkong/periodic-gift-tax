    ...
    <section class="print-page">
      ...
      <table class="doc-table">
        <tbody>
          ...
          <tr><th>PDF 구성</th><td>신고서 초안, 증여재산 및 평가명세서 초안, 선택 보관용 확인서</td></tr>
      ...
      <p class="doc-footnote">
        ...
        가족관계 및 10년 내 증여 내역을 확인해야 합니다. 약정서나 확인서는 사실관계 정리용 보관자료이며,
        모든 신고에서 필수 첨부서류라는 뜻은 아닙니다.
      </p>
    </section>
  `;
}
...
function renderPropertyStatement(input, valuation, tax) {
  const valuationMethod = input.giftMode === "lump_sum"
    ? "현금 일시증여 평가"
    : "유기정기금 현재가치 평가";
  const scheduleRows = valuation.schedule
    .map(
      (row) => `
        <tr>
    ...
    .join("");
  const valuationDetail = input.giftMode === "periodic"
    ? `
      <h3>유기정기금 평가 근거</h3>
...
function renderAgreement(input, valuation) {
  return `
    <section class="print-page">
      ...
      <p class="doc-footnote">
        이 약정서는 정기증여 조건을 정리해두기 위한 선택 보관자료입니다. 실제 제출 필요 여부는 홈택스 첨부화면과 관할 세무서 안내를 따릅니다.
      </p>
...
function renderCashAgreement(input, valuation) {
  return `
    <section class="print-page">
      ...
      <p class="doc-footnote">
        이 확인서는 현금 증여 사실을 정리해두기 위한 선택 보관자료입니다. 실제 제출 필요 여부는 홈택스 첨부화면과 관할 세무서 안내를 따릅니다.
      </p>
      <div class="signature-line">
        <span>증여자: ${escapeHtml(input.donorName) || blank()} (서명)</span>
        <span>법정대리인: ${escapeHtml(input.guardianName || input.donorName) || blank()} (서명)</span>
